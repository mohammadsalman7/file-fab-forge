...
export const PdfPasswordRemover = () => {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      toast.info(requiresPassword ? 'This PDF is password protected. You can remove the password.' : 'This PDF is not password protected.');
    } catch (error) {
  ...
  const handleProcessPassword = async () => {
    ...
    if (!password.trim()) { toast.error('Please enter the current PDF password.'); return; }
              ...
              <div className="space-y-4">
                {isPasswordRequired ? (
                  ...
                  <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">You can still proceed to re-save it without password</p>
                  </div>
                )}
